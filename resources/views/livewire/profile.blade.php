            ...
            <form wire:submit.prevent="updateProfile">
                <div class="space-y-4">
                    <div>
                        ...
                        <x-form.input id="name" name="name" wire:model="name" />
                        @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <x-form.label for="email">Email</x-form.label>
                        <x-form.input type="email" id="email" name="email" wire:model="email" />
            ...
            <form wire:submit.prevent="updatePassword">
                <div class="space-y-4">
                    <div>
                        ...
                        <x-form.input type="password" id="current_password" name="current_password" wire:model="current_password" />
                        @error('current_password') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <x-form.label for="password">New Password</x-form.label>
                        <x-form.input type="password" id="password" name="password" wire:model="password" />
                        @error('password') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <x-form.label for="password_confirmation">Confirm New Password</x-form.label>
                        <x-form.input type="password" id="password_confirmation" name="password_confirmation" wire:model="password_confirmation" />
                    </div>
                    <div>
